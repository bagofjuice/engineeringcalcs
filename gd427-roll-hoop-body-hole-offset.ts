...
const visualiseHolePositions = (
  ...
  // ------------------- OUTER --------------------
  // Outer plumb hole
  drawCirle(ctx, FRONT_LEG_DIA / 2, (type === 'nearside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X), HOLE_FRONT_Y, "hole");

  // Outer offset
  drawCirle(
    ctx,
    FRONT_LEG_DIA / 2,
    (type === 'nearside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X) + rollHoop.outerLeg.nsToOs.absolute,
    HOLE_FRONT_Y + rollHoop.outerLeg.frontToBack.absolute,
    "leg"
  );
  addLabel(
    ctx,
    (type === 'nearside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X) + FRONT_LEG_DIA / 1.5,
    HOLE_FRONT_Y,
  ...
  // ------------------- INNER --------------------
  // Inner plumb hole
  drawCirle(ctx, FRONT_LEG_DIA / 2, (type === 'offside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X), HOLE_FRONT_Y, "hole");

  // Outer offset
  drawCirle(
    ctx,
    FRONT_LEG_DIA / 2,
    (type === 'offside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X) + rollHoop.innerLeg.nsToOs.absolute,
    HOLE_FRONT_Y + rollHoop.innerLeg.frontToBack.absolute,
    "leg"
  );
  addLabel(
    ctx,
    (type === 'offside' ? HOLE_FRONT_LEFT_X :  HOLE_FRONT_RIGHT_X) + FRONT_LEG_DIA / 1.5,
    HOLE_FRONT_Y,
